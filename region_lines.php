...
$crimea = array();
if ($region == "crimeaua") {
  $query = "SELECT old_esr,esr FROM crimea";
...
?>
<a href='./region:<?php echo $output["region_code"]; ?>:a'>По алфавиту</a> | <b>По участкам</b> | <a href="legend">Легенда</a><p>
  ...
  <tr>
    <td style='background-color: <?php echo $color[$output_row["status"]]; ?>'>
	  ...
	  $tmp = "<strike>$tmp</strike>";
        ...
        if ($output_row["km"])
          echo "<br/><font size=-1>".$output_row["km"]."</font>";
      ?>
    </td>
    <td style='background-color: <?php echo $color[$output_row["status"]]; ?>'>
      <?php
	...
	if (count($osmnodes) > 0) {
	  echo implode("\n", $osmnodes);
	    ...
	    echo "<strike><a href=./esr:".$output_row["dup_esr"].">ЕСР: ".$output_row["dup_esr"]."</a></strike>";
          elseif (isset($crimea[$output_row["esr"]]) && $crimea[$output_row["esr"]])
            echo "<a href=./esr:".$crimea[$output_row["esr"]].">ЕСР: ".$crimea[$output_row["esr"]]."</a>";
	  else
	    echo "&nbsp;";
	  }
      ?>
